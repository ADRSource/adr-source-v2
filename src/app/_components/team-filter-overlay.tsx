    ...
    <Drawer.Root open={open} onOpenChange={onOpenChange}>
      ...
      <Drawer.VirtualKeyboardProvider>
        <Drawer.Portal keepMounted>
          <Drawer.Backdrop className="fixed inset-0 z-40 min-h-dvh bg-brand-black/70 opacity-[calc(1-var(--drawer-swipe-progress))] transition-opacity duration-200 ease-[cubic-bezier(0.32,0.72,0,1)] data-[ending-style]:opacity-0 data-[starting-style]:opacity-0 data-[swiping]:duration-0 supports-[-webkit-touch-callout:none]:absolute [&[hidden]]:hidden" />
          <Drawer.Viewport className="fixed inset-0 z-40 flex items-end justify-center [&[hidden]]:hidden">
            <Drawer.Popup className="flex h-[85dvh] w-full flex-col rounded-t-2xl border border-b-0 border-brand-copper bg-brand-black/90 text-brand-copper shadow-lg outline-none backdrop-blur-sm backdrop-saturate-150 transition-transform duration-200 ease-[cubic-bezier(0.32,0.72,0,1)] will-change-transform [transform:translateY(var(--drawer-swipe-movement-y))] data-[swiping]:select-none data-[ending-style]:duration-200 data-[swiping]:duration-0 data-[ending-style]:[transform:translateY(calc(100%+2px))] data-[starting-style]:[transform:translateY(calc(100%+2px))] [&[hidden]]:hidden">
              <div
                aria-hidden="true"
              ...
              <Drawer.Content className="flex min-h-0 flex-1 flex-col overflow-hidden p-[calc(theme(spacing.1)*2)] pb-[max(theme(spacing.2),env(safe-area-inset-bottom,0px))]">
                {panel('sheet')}
              </Drawer.Content>
            </Drawer.Popup>
          </Drawer.Viewport>
